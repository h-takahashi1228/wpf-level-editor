using MapEditTool.Rendering3D;
using MapEditTool.Models.Map;

namespace MapEditTool.Scene3D
{
    public class DefenseBaseOutlineScene : ObjectOutlineSceneBase
    {
        public DefenseBaseOutlineScene(GridPos pos, int yaw)
            : base(pos, yaw)
        {
            var renderer = new DefenseBaseOutlineRenderer();
            Model = renderer.Render();

            Model.Transform = CreateTransform(pos, yaw);
        }

        public override void Update(GridPos pos, int yaw)
        {
            base.Update(pos, yaw);
        }
    }
}
